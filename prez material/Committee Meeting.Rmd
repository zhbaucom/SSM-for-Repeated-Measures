...
* Working with Professor Yorghos Tripodis PhD. 
...
* We can think of autocorrelation as the effect of unobserved time-varying variables (UTV).
* Different modeling techniques make different assumptions on the behavior of the UTV
  * LMEM: There is not effect over time of UTV.
  * LMEM AR(1): The UTV effect will revert back to their levels at baseline.
  * LLT: The UTV can vary freely overtime for each subject.
...
We can think of $\alpha_t$ as the underlying cognitive state not accounted for by the covariates $X_t$. 
...
outb <- lapply(flist, function(x){
 Vars <- x %>% 
  ...
  VarBias %>%
    ...
    mutate(
      Vars = case_when(
        grepl("None", x) ~ "$\\sigma^2 = 1$, $\\rho = 0$",
        grepl("Small", x) ~ "$\\sigma^2 = 1$, $\\rho = 0.1$",
        grepl("Medium", x) ~ "$\\sigma^2 = 1$, $\\rho = 0.5$",
...
ggplot(outb, aes(x = Method, y = Bias, fill = Method)) + 
  ...
  scale_fill_manual(values = c("#405E92", "#408B92",   "#DF3116",   "#DF8016",  "#DFB916", "#FFEA60", "#409247"))
...
outci %>%
  ...
  ggplot(aes(x = Method, y = CI.Length, color = Method)) + 
  stat_summary(fun=median, geom="point", shape=18, size=5) +
  facet_wrap((Vars) ~ ., scales = "free_y",
  labeller=label_parsed)+
  theme_classic()+
...
* The state space methods give unbiased estimates while maintaining near 0.95 coverage probability for the 95% CIs.
  * While the LME methods are unbiased, they do not maintain 0.95 coverage probability under model mispecification.
...
```{r, cache = TRUE}
DateFolder <- paste("Simulations/RealDataSim/", "03252021", "/",sep = "") 
...
outSim[,1,] %>%
  ...
  mutate(Method = factor(Method, levels = c("lme", "ar1", "part", "bayes"))) %>%
  ggplot(aes(x = Method, y = Value, fill = Method)) + 
    ...
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank()) +
    scale_fill_manual(values = c("#405E92", "#408B92", "#DF9816", "#409247")) +
  theme_classic()
...
row.names(ModEs) <- c("lme", "ar1", "bayes")
