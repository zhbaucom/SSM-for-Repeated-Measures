...
KalmanRegEffec <- function(y, X, Beta.Initial = NULL, sigma2.eps = 1, sigma2.eta = 1, a1 = 0, P1 = 1e7, P1B = NULL, Lik = FALSE){
  # logVar <- c(0.1, -20.9)
  ...
  T <- rbind(cbind(diag(1, n, n), X), cbind(matrix(0, p, n), diag(1, p, p)))
  ...
  Z <- cbind(diag(1, n, n), matrix(0, n, p))
  ...
  K[[1]] <- T %*% tcrossprod(P[[1]], Z.Star) %*% F.inv[[1]]
  L[[1]] <- T - K[[1]] %*% Z.Star
  ...
  for(i in 2:NS){
    ...
    if(CalcP){
      P[[i]] <- T %*% tcrossprod(P[[i-1]],L[[i-1]]) + Q
      ...
      K[[i]] <- T %*% tcrossprod(P[[i]], Z.Star) %*% F.inv[[i]]
      L[[i]] <- T - K[[i]] %*% Z.Star
