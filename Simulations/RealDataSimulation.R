...
if(!(dir.exists(paste("Simulations/RealDataSim/",td, sep = ""))))
  dir.create(paste("Simulations/RealDataSim/",td, sep =""))
...
longdata <- longdata1 %>%
  ...
  select(NACCID, time,Intercept, ANIMALS, SEX, EDUC, RACEWHITE, AgeBase, DEC, APOE, APOESEX) %>%
  na.omit() %>%
...
##Setup Data
ld2 <- longdata  %>%
  # mutate(SEX = SEX * time, RACEWHITE = RACEWHITE * time, APOE = (APOE*2-1) * time, APOESEX = (APOESEX *2 -1))
...
for(i in 1:Sims){
      ...
      ld2$A2 <- sd1 * ld2$BinInt + ld2$ANIMALS
    ...
    tryCatch(
      ...
      {  
      ###LME
      lmeMod <- lme(A2 ~ (time +SEX + EDUC + RACEWHITE + AgeBase + DEC + APOE + APOESEX + BinInt), random =  ~1|NACCID, data = ld2, method = "ML")
    ...
    tryCatch(
      ...
      { 
      ###LME AR(1)
      lmeMod <- lme(A2 ~ (time +SEX + EDUC + RACEWHITE + AgeBase + DEC + APOE + APOESEX + BinInt), correlation = corAR1(form = ~time), random =  ~1|NACCID, data = ld2, method = "ML")
      CIout <- intervals(lmeMod, which = "fixed")$fixed[10,]
    ...
    tryCatch(
      ...
      saveRDS(OutArray, paste("Simulations/RealDataSim/",td,"/NEWoutArray_",l,".RDS", sep = ""))
      saveRDS(VarArray, paste("Simulations/RealDataSim/",td,"/NEWVarArray_",l,".RDS", sep = ""))
