NBSim <- function(dataType = "SSM", simXfun = NULL, ...){
  ...
  ##################LME#############################
  LMEform <- paste("y ~ (", paste(colnames(XY)[grepl("X", colnames(XY))], collapse = " + "), ") + (1|id) - 1", sep = "")
  lmeout <- suppressMessages(lmer(as.formula(LMEform), data = XY, REML = FALSE))
  lmesum <- summary(lmeout)
  LMECI <- suppressMessages(confint(lmeout)[-c(1:2),])
  
  EstMat[,1] <- lmesum$coefficients[,1]
  uclMat[,1] <- LMECI[,2]
  lclMat[,1] <- LMECI[,1]
  ...
  #########################AR1##############################
  ARform <- paste("y ~ (", paste(colnames(XY)[grepl("X", colnames(XY))], collapse = " + "), ")-1", sep = "")
  ...
  okout <- KalmanReg(y = y, X = X, Beta.Initial = Beta.Initial, a1 = u0, P1 = 1e7)
  ...
  ###################### Run Original Kalman Filter ##################
  pkout2 <- KalmanRegPart(y = y, X = X, Beta.Initial = Beta.Initial, a1 = u0, P1 = 1e7, groups = 2)
  UCL <-  pkout2$Estimate + qnorm(.975) * sqrt(diag(pkout2$Variance))
  LCL <- pkout2$Estimate + qnorm(.025) * sqrt(diag(pkout2$Variance))

  EstMat[,5] <- pkout2$Estimate
  uclMat[,5] <- UCL
  lclMat[,5] <- LCL
  sigma2Mat[,5] <- c(pkout2$sigma2.eps, pkout2$sigma2.eta)

  ############ Run Original Kalman Filter ###################
  pkout4 <- KalmanRegPart(y = y, X = X, Beta.Initial = Beta.Initial, a1 = u0, P1 = 1e7, groups = 4)
  UCL <-  pkout4$Estimate + qnorm(.975) * sqrt(diag(pkout4$Variance))
  LCL <- pkout4$Estimate + qnorm(.025) * sqrt(diag(pkout4$Variance))

  EstMat[,6] <- pkout4$Estimate
  uclMat[,6] <- UCL
  lclMat[,6] <- LCL
  ...
  ################### Run Original Kalman Filter ####################
  pkout10 <- KalmanRegPart(y = y, X = X, Beta.Initial = Beta.Initial, a1 = u0, P1 = 1e7, groups = 10)
  UCL <-  pkout10$Estimate + qnorm(.975) * sqrt(diag(pkout10$Variance))
  LCL <- pkout10$Estimate + qnorm(.025) * sqrt(diag(pkout10$Variance))

  EstMat[,7] <- pkout10$Estimate
  uclMat[,7] <- UCL
  lclMat[,7] <- LCL
...
NBdataSim <- function(){
  ...
  # y <- t(apply(ytot, 1, function(x){
  #   x * c(1, sample(c(NA, 1), length(x)-1, replace = TRUE))
  # }))
  ...
  #   nMeas <- sample(c(4,4), 1)
  #   sort(c(1, sample(2:5, nMeas)))
  # })
  # 
  # 
  ...
  XY <- XYs %>%
    group_by(id) %>%
    # mutate(Keep = c(TRUE, 2:n() %in% sample(2:n(), sample(1:4, 1)))) %>%
    mutate(Keep = TRUE) %>%
    filter(Keep == TRUE) %>%
    ungroup(id) %>%
    arrange(id, time) 
  ...
  list(X = X, y = y, XY = XY, y.long = y.long, X.long = X.long, id = id, time = time)
...
Compiler <- function(reps = 100, dataType, simXfun, ...){
  ...
  for(i in 1:reps){
    ...
    while(Again){
      ...
      if(class(SimOut) == "list"){ Again <- FALSE }else{ Fails <- Fails + 1; Fcount <- Fcount + 1}
      if(Fcount >= 5)break  
    }
