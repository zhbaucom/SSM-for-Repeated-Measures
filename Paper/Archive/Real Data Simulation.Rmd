...
(outSim[,2,] < Bval & outSim[,3,] > Bval) %>%
  ...
  kbl(longtable = TRUE, escape = FALSE, caption = "Simulated parameter coverage") %>%
  kable_styling()
...
outSim[,1,] %>%
  ...
  ggplot(aes(x = Method, y = Value, fill = Method, shape = Method)) + 
    geom_violin() + 
